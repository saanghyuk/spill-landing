...
export default function Punchline({ line, sub }: PunchlineProps) {
  ...
  return (
    <div ref={ref} className="py-20 md:py-32 px-6 relative">
      ...
      >
        <p className="text-2xl sm:text-3xl md:text-4xl font-bold text-foreground leading-snug">
          {line}
        </p>
        {sub && <p className="text-base sm:text-lg text-gray mt-4">{sub}</p>}
      </motion.div>
    </div>
  );
}
